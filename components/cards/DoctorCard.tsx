...
export default function DoctorCard({
  ...
  const formattedPrice = new Intl.NumberFormat(lang === 'kz' ? 'kk-KZ' : 'ru-RU').format(price);
  ...
  return (
    ...
    >
      <div className="relative h-56 w-full shrink-0">
        <Image src={image || "/images/placeholder.png"} alt={name || "Doctor"} fill className="object-cover" />
      ...
      <div className="flex flex-col flex-grow p-6">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-4 text-xs font-medium text-gray-500">
          {rating && (
            <div className="flex items-center gap-1 text-gray-900">
              ...
              <span className="text-gray-400 font-normal ml-1">{reviewsCount} {t('reviews')}</span>
            </div>
          )}
          {experienceYears && (
        ...
        {features && features.length > 0 && (
          <div className="flex flex-col gap-2 mb-4">
            {features.map((feature, idx) => (
              <div key={idx} className="bg-blue-50 text-blue-700 text-xs font-medium px-3 py-2 rounded-xl">
                {feature}
              </div>
            ))}
          </div>
        ...
        {quote && (
          <div className="bg-gray-50 text-gray-500 text-xs italic p-4 rounded-xl mb-4">
            &quot;{quote.text}&quot;
            <span className="block mt-2 not-italic font-medium">— {quote.author}</span>
          </div>
        )}
        ...
        <div className="flex items-end justify-between mt-4 pt-4 border-t border-gray-50">
          <div>
            <p className="text-[10px] text-gray-400 uppercase tracking-wider mb-0.5">{t('price')}</p>
