  ...
  return (
    <header className="relative flex items-center justify-between px-4 md:px-8 py-3 md:py-4 bg-white shadow-sm font-sans z-50">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/${locale}`} className="flex items-center gap-2 md:gap-3" onClick={closeMenu}>
          ...
          <div className="flex flex-col">
            <span className="text-lg md:text-xl font-bold leading-none">DentDoctor</span>
      ...
      <div className="flex items-center gap-3 md:gap-6">
        ...
        <div className="hidden lg:flex items-center gap-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <span className="font-bold text-sm">+7 (705) 279-30-78</span>
      ...
      {isMobileMenuOpen && (
        <div className="absolute top-full left-0 w-full bg-white border-t border-gray-100 shadow-lg lg:hidden flex flex-col px-4 py-6 gap-6 z-50">
          <nav className="flex flex-col gap-4 text-base font-medium text-gray-800">
            <Link href={`/${locale}`} onClick={closeMenu} className="hover:text-blue-600">{t('home')}</Link>
            <Link href={`/cases`} onClick={closeMenu} className="hover:text-blue-600">{t('cases')}</Link>
            <Link href={`/blog`} onClick={closeMenu} className="hover:text-blue-600">{t('blog')}</Link>
            <Link href={`/${locale}/about`} onClick={closeMenu} className="hover:text-blue-600">{t('about')}</Link>
          </nav>

          <div className="w-full h-px bg-gray-100" />

          <div className="flex flex-col gap-4">
            <Button asChild className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl w-full">
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <div className="flex flex-col">
                <span className="font-bold text-sm text-gray-900">+7 (705) 279-30-78</span>
                <span className="text-[10px] text-gray-400 uppercase tracking-wider">
                  {t('phoneSub')}
                </span>
              </div>
