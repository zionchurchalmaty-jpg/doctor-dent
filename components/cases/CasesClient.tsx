...
export default function CasesClient({ initialCases, doctorsList, clinicsList }: CasesClientProps) {
  ...
  const filteredCases = useMemo(() => {
    return initialCases.filter((item) => {
      const matchDoctor = selectedDoctor ? item.doctorId === selectedDoctor : true;
      const matchClinic = selectedClinic ? item.clinicName === selectedClinic : true; 
      ...
      <div className="max-w-7xl mx-auto mt-12">
        <div className="mb-6">
          <h2 className="text-gray-600 text-sm font-medium uppercase tracking-wider">
            Найдено кейсов: <span className="font-bold text-gray-900">{filteredCases.length}</span>
          </h2>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCases.map((item) => (
              <CaseCard 
                key={item.id}
                id={item.id}
                category="Кейс лечения" 
                title={item.title?.ru || item.title || "Без названия"} 
                doctorName={item.doctorName}
                doctorSlug={item.doctorSlug}
                problem={item.description?.ru || item.description || "Описание отсутствует"}
                duration={item.duration?.ru || item.duration || "-"}
